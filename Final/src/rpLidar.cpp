...
int rpLidar::getMeasurePoints(int16_t count)
{
    ...
    {
        scanDot dot;
        if (!_cached_scan_node_hq_buf[pos].dist_mm_q2)
            continue;
        ...
        if ((double)dot.dist > farthest_point_angle_distance)
        {
            if ((int)dot.angle <= 80)
            {
                farthest_point_angle = (int)dot.angle;
                farthest_point_angle_distance = (double)dot.dist;
            }
            else if ((int)dot.angle >= 280)
            {
                farthest_point_angle = (int)dot.angle - 360;
                farthest_point_angle_distance = (double)dot.dist;
            }
        }
    }
